 ...
 * 和一个实现 {@link OpenClawPlugin} 接口的 Java 类。
 * <p>
 * 热插拔机制：
 * <ul>
 *   <li>扫描时发现新插件 → 自动注册</li>
    ...
    private void loadPluginClass(Path dir, String pluginId, PluginRegistrationApi api) {
        ...
        try {
            ...
            if (mainClass != null && !mainClass.isBlank()) {
                ...
                if (instance instanceof OpenClawPlugin plugin) {
                    plugin.register(api);
                    log.info("[PluginLoader] Plugin '{}' class loaded: {}", pluginId, mainClass);
                } else {
                    log.warn("[PluginLoader] Plugin '{}' class {} does not implement OpenClawPlugin", pluginId, mainClass);
                }
            }

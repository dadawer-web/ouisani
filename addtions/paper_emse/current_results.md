...
## Fixed local-model anchor

The fixed-model runner is `evaluation/reflexion_ollama_qwen3_anchor.py` and
supports an explicit model argument. The accepted open-model sensitivity
anchor is the cached local Ollama `qwen:7b`, whose Qwen2-family identity and
digest are recorded in its raw log. The attempted `qwen3:8b` pull failed before
manifest download with `registry.ollama.ai: no such host`; the resulting blocker record is
`evaluation/results/emse_reflexion_qwen3_ollama_anchor/QWEN3_BLOCKER.md`.
No Qwen3-8B numerical claim is made.

## Exploratory cached local anchor (not Qwen3)

The cached Ollama `qwen:7b` model (Ollama family `qwen2`) completed a bounded
...
Fisher's exact `p=0.3635`. This is the paper's fixed open-model sensitivity
anchor, reported as exploratory rather than as a powered mitigation result.

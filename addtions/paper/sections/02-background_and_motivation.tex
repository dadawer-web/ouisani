...
surfaces introduced by LIM agents (Section~\ref{sec:bg-attack}), and derives the
design goals that motivate Neuron (Section~\ref{sec:bg-goals}).

% -----------------------------------------------------------------------------
\subsection{Multi-Tenant LLM Agent Operating Systems}
\label{sec:bg-kernel}

An \emph{LLM agent operating system} extends the classical process abstraction
...
processes and elevate them to first-class OS concerns:
...
Classical operating systems isolate concerns across three largely independent
layers, each with its own mechanism and its own audit trail:

\begin{itemize}
    \item \textbf{Resource control} (cgroups~\cite{linux2024cgroupv2}) accounts
    for and caps resource consumption---CPU, memory, and, in our setting, LLM
    tokens. Resource decisions are recorded in cgroup-local logs.
    ...
    \item \textbf{Access control} (capabilities~\cite{mercer2012capsicum,
    klein2009sel4}, declarative policies) determines whether an operation is
...
out-of-memory signal, there exists a transient \emph{contention window} in
...
Without a single trace that threads through cgroup, sandbox, and permission
decisions, cross-layer correlation is impossible: the runtime cannot reconstruct
that ``the permission grant at time $t$ occurred inside the contention window
opened by the broadcast flood at time $t-\delta$.'' Each layer operates---and fails---in isolation.

% -----------------------------------------------------------------------------
\subsection{Novel Attack Surfaces in LIM Agents}
\label{sec:bg-attack}

Beyond the coupled-failure mode, LIM agents introduce attack surfaces that
...
Goals G1 and G5 target the coupled-failure mode of Section~\ref{sec:bg-lim}; goals
G2--G4 target the LIM-specific attack surfaces of Section~\ref{sec:bg-attack}; G6
constrains the design space to evolvable, non-disruptive closures. The Neuron
architecture presented in Section~\ref{sec:arch} is designed to satisfy these
goals jointly, rather than in the layer-separated fashion that motivates this
work.

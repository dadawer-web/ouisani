#!/usr/bin/env python3
"""Run a fixed local Ollama Reflexion TrustOrigin sensitivity anchor.
...
def main() -> int:
    ...
    model_slug = re.sub(r"[^a-z0-9]+", "_", OLLAMA_MODEL.lower()).strip("_")
    anchor_prefix = f"{model_slug}_anchor"
    raw_path = out / f"{anchor_prefix}.raw.jsonl"
    summary_path = out / f"{anchor_prefix}.json"
    checkpoint_path = out / f"{anchor_prefix}.checkpoint.jsonl"

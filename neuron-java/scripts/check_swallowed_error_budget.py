#!/usr/bin/env python3
"""Enforce a ratcheting budget for swallowed-error-like Java patterns.

借鉴 jcode/scripts/check_swallowed_error_budget.py，适配 Java。

Tracks production occurrences of patterns that commonly hide failures and
should either be removed, logged, propagated, or explicitly accepted:

- `catch (Exception`     — broad exception swallowing
- `catch (Throwable`     — catching Throwable (catches OOM, etc.)
- `.printStackTrace()`   — should use logger instead
- empty catch block      — `catch (...) { }` or comment-only body
...
BASELINE_FILE = REPO_ROOT / "scripts" / "budgets" / "swallowed_error_budget.json"
SCAN_ROOTS = (REPO_ROOT / "src" / "main" / "java",)

PATTERNS = {
    "catch_exception": re.compile(r"\bcatch\s*\(\s*Exception\b"),
    "catch_throwable": re.compile(r"\bcatch\s*\(\s*Throwable\b"),
    "print_stack_trace": re.compile(r"\.printStackTrace\s*\(\s*\)"),
}
        ...
        for line in cleaned.splitlines():
            for name, pattern in PATTERNS.items():
                if pattern.search(line):
                    file_counts[name] += 1
        # 空 catch 块需多行匹配
        empty_catch_count = len(EMPTY_CATCH_RE.findall(cleaned))
        if empty_catch_count:
            file_counts["empty_catch"] = empty_catch_count
...
def total_counts(counts: dict[str, dict[str, int]]) -> dict[str, int]:
    totals = zero_counts()
    # 确保 empty_catch 键存在
    totals.setdefault("empty_catch", 0)
...
def load_baseline() -> dict[str, Any]:
    if not BASELINE_FILE.exists():
        return {
            "version": 1,
            "total": 0,
            "totals_by_pattern": {**zero_counts(), "empty_catch": 0},
            "tracked_files": {},
        }
...
def main() -> int:
    ...
    baseline_pattern_totals: dict[str, int] = baseline.get("totals_by_pattern", {})

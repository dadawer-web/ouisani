...
public class PermissionStarvationJvmBenchmark {
    ...
    /**
     * Factorial ablation configuration.  All four cells use fail-closed
     * semantics so that the experiment isolates capacity reservation and the
     * cross-layer pressure signal rather than mixing in fallback behavior.
     */
    record AblationConfig(String name, boolean isolatedPermissionPool, boolean coupled) {}

    private static final List<AblationConfig> ABLATION_CONFIGS = List.of(
            new AblationConfig("shared_no_signal", false, false),
            new AblationConfig("shared_signal", false, true),
            new AblationConfig("isolated_no_signal", true, false),
            new AblationConfig("isolated_signal", true, true));
    ...
    void runJvmStarvationResponseSurface() throws Exception {
        ...
        } else if (MODE.equals("ablation_2x2")) {
            raw.addAll(runAblationMatrix());
    ...
    private List<Map<String, Object>> runAblationMatrix() throws Exception {
        List<Map<String, Object>> rows = new ArrayList<>();
        Load[] loads = {
                new Load(1, 100, 0.5, "ablation", "low-contention", "attack"),
                new Load(4, 500, 2.0, "ablation", "medium-contention", "attack"),
                new Load(16, 2_000, 5.0, "ablation", "high-contention", "attack")
        };
        for (Load load : loads) {
            for (AblationConfig config : ABLATION_CONFIGS) {
                rows.addAll(runAblationCell(config, load, 10));
            }
        }
        return rows;
    }

    private List<Map<String, Object>> runAblationCell(
            AblationConfig config, Load load, int deadlineMs) throws Exception {
        ThreadPoolExecutor shared = pool(SHARED_POOL_SIZE, "ablation-shared");
        ThreadPoolExecutor permission = config.isolatedPermissionPool
                ? pool(PERMISSION_POOL_SIZE, "ablation-permission") : shared;
        ...
        for (int i = 0; i < load.attackers; i++) {
            Thread producer = Thread.ofVirtual().name("ablation-attacker-" + i).start(() -> {
        ...
        for (int trial = 0; trial < DECISIONS_PER_CELL; trial++) {
            ...
            try {
                Future<EscalationPolicy.Verdict> future = permission.submit(() ->
                        EscalationPolicy.evaluate(2, "bash", "", resourceRejections.get(), config.coupled));
                try {
                    verdict = future.get(deadlineMs, TimeUnit.MILLISECONDS).name();
                } catch (TimeoutException timeout) {
                    timedOut = true;
                    future.cancel(true);
                    verdict = "DENY_TIMEOUT";
                }
            ...
            row.put("experiment", "ablation_2x2");
            ...
            row.put("architecture", config.name);
            row.put("capacity_isolated", config.isolatedPermissionPool);
            row.put("pressure_signal", config.coupled);
            row.put("trial", trial);
